...
# 获取老师模型输出
def fetch_teacher_output(t_model, data_iter):
    # eval
    t_model.eval()
    output_list = []
    # 预测
...
# KL散度损失，要求输入stu_log_probability,teacher_probability
...
def train_kd(cnn_config, bert_model, cnn_model, bert_train_iter, cnn_train_iter, cnn_dev_iter, cnn_test_iter):
    """
    ...
    start_time = time.time()
    # 获取CNN模型参数
    param_optimizer = list(cnn_model.named_parameters()) # 获取模型可训练参数的名称和参数本身的键值对,是一个迭代器
    no_decay = ['bias', 'LayerNorm.bias', 'LayerNorm.weight']
    optimizer_grouped_parameters = [
