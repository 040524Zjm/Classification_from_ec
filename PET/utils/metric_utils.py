...
class ClassEvaluator(object):
    ...
    def add_batch(self,pred_batch:List[List],gold_batch:List[List]):
        ...
        :param pred_batch: 模型预测标签列表, e.g. ->  [['体', '育'], ['财', '经'], ...]
        :param gold_batch: 真实标签标签列表, e.g. ->  [['体', '育'], ['财', '经'], ...]
        """
        assert len(pred_batch) == len(gold_batch) # 强制检查
        # print(f'pred_batch0--》{pred_batch}')
        # print(f'gold_batch0--》{gold_batch}')
        if type(gold_batch[0]) in [list, tuple]:  # 若遇到多个子标签构成一个标签的情况
            pred_batch = [''.join([str(e) for e in ele]) for ele in pred_batch] # 拼成一个
    ...
    def compute(self,round_num=2) -> dict:
        ...
        res['precision'] = round(precision_score(self.goldens, self.predictions, average='weighted'), round_num) # 加权平均
        ...
        # precision 预测该类且预测对，recall预测为真中预测对的个数，f1 = 2 * precision * recall / (precision + recall)  //f-t n-p
        try:
