...
def convert_example(
    ...
    :param tokenizer:
    :param max_seq_len:句子的最大长度，若没有达到最大长度，则padding为最大长度
    :param max_label_len:
    :param hard_template:硬模版
    :param train_mode:训练阶段 or 推理
    :param return_tensor:是否返回tensor？否则numpy
    ...
    for i, example in enumerate(examples['text']):
        if train_mode:
            ...
            label, content = example.strip().split('\t') # '手机	这个手机也太卡了。'
            # print(f'label-->{label}')
            # print(f'content-->{content}')

        else:
            content = example.strip() # '这个手机也太卡啦'
        inputs_dict = {
            'textA': content,
            'MASK': '[MASK]'
        }
        ...
        if train_mode:
            label_encoded = tokenizer(text=[label]) # tokenizer会补到最大长度
...
if __name__ == '__main__':
    ...
    print('*'*80)
    print(f'train_dataset[\'train\'][\'text\']:')
    ...
    #
    # print(f'tokenized_output-->{tokenized_output}')
    convert_func = partial( # partial函数可以固定某些参数，返回一个函数
        convert_example,
        tokenizer=tokenizer,
        hard_template=hard_template,
        max_seq_len=30,
